...
以目前畫面品質h264儲存檔案約1MB, 若降低視頻畫面品質, 則存儲記憶體將可以存儲更多資料, 或是增加USB Disk容量都可以將存放的天數增加。外部存儲裝置USB disk 4GB空間可存放視頻檔案(.h264)資料循環存放約可保留最近4天(存儲空間不足時會同時卡住Video Stream及將會遺失視頻資料)。

過期天數的視頻資料刪除是利用Bash Script語言編輯的, 而運行機制是crontab -e 編輯安排在每小時檢查此Bash Script 意思是每小時檢查天數是否過期進而進行刪除。亦可更細緻的調整檢查時間, 可讓存儲空間發揮更大效用。
